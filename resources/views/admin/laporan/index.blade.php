...
<div class="page-header mb-4">
    <h3 class="mb-1">Laporan Peminjaman Buku</h3>
    <p class="text-muted mb-0">Filter dan export riwayat peminjaman buku</p>
</div>
...
<div class="card mb-3">
    <div class="card-body">
        ...
        <form method="GET" class="row g-3">
            ...
            <div class="col-md-3">
                <label class="form-label small">Buku</label>
                <select name="buku_id" class="form-select">
                    <option value="">Semua</option>
                    @foreach($bukus as $buku)
                    <option value="{{ $buku->id }}" @selected(request('buku_id')==$buku->id)>
                        {{ $buku->judul }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="col-12 d-flex justify-content-between align-items-end mt-2">
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('admin.laporan.index') }}" class="btn btn-light border btn-sm">
                        Reset
                    </a>
                </div>

                <div class="d-flex gap-2">
                    <a href="{{ route('admin.laporan.pdf', request()->all()) }}" class="btn btn-danger btn-sm">
                        Export PDF
                    </a>

                    <a href="{{ route('admin.laporan.excel', request()->all()) }}" class="btn btn-success btn-sm">
                        Export Excel
                    </a>
                </div>
            </div>
